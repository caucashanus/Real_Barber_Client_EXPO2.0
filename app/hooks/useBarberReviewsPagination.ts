...
export function useBarberReviewsPagination(
  ...
  const reviewIdsKey = initialReviews.map((review) => review.id).join('|');
  const seedKey = `${employeeId ?? ''}:${reviewIdsKey}`;
  ...
  const goNext = useCallback(async () => {
    if (!employeeId || !canGoNext) return;
    ...
    try {
      ...
      while (merged.length < requiredCount && merged.length < totalReviews) {
        const data = await getPublicEntityReviews('employee', employeeId, {
  ...
  }, [allReviews, canGoNext, employeeId, page, totalReviews]);
